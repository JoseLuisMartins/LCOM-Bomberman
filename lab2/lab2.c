...
static void print_usage(char *argv[]) {
  printf("Usage: one of the following:\n"
	 "\t service run %s -args \"timer_test_square <decimal number>\" \n"
	 "\t service run %s -args \"timer_test_config <decimal number>\" \n"
	 "\t service run %s -args \"timer_test_int <decimal number>\" \n",
...
static int proc_args(int argc, char *argv[]) {
 ...
 if (strncmp(argv[1], "timer_test_square", strlen("timer_test_square")) == 0) {
	  ...
	  if (timer_test_square(freq)){
		  printf("Error on timer_test_square()\n");
		  return 1;
	  }
	  return 0;
	  ...
	  if (timer_test_config(timer))
	  {
		  printf("Error on timer_test_config()\n");
		  return 1;
	  }
	  return 0;
	  ...
	  {
		  printf("Error on timer_test_int()\n");
		  return 1;
	  }
	  return 0;
  } else {
